...
if killer == nil then
	--print ( '[KILLER] creating killer game mode' )
	killer = class({})
end
...
function killer:OnHeroInGame(hero)
	...
	if not self.greetPlayers then
	    ...
	    local firstLine = ColorIt("Welcome to ", "green") .. ColorIt("killer! ", "magenta") .. ColorIt("v0.1", "blue");
    ...
    ShowGenericPopupToPlayer(hero.player, "#killer_instructions_title", "#killer_instructions_body", "", "", DOTA_SHOWGENERICPOPUP_TINT_SCREEN )
...
function killer:OnGameInProgress()
	--print("[KILLER] The game has officially begun")
...
function killer:PlayerSay( keys )
	local ply = keys.ply
...
function killer:OnGameRulesStateChange(keys)
	--print("[KILLER] GameRules State Changed")
	--PrintTable(keys)
...
function killer:OnItemPickedUp(keys)
	--print ( '[KILLER] OnItemPurchased' )
	--PrintTable(keys)
...
function killer:OnItemPurchased( keys )
	--print ( '[KILLER] OnItemPurchased' )
	--PrintTable(keys)
...
function killer:OnAbilityUsed(keys)
	--print('[KILLER] AbilityUsed')
	--PrintTable(keys)
...
function killer:OnNonPlayerUsedAbility(keys)
	--print('[KILLER] OnNonPlayerUsedAbility')
	--PrintTable(keys)
...
function killer:OnPlayerChangedName(keys)
	--print('[KILLER] OnPlayerChangedName')
	--PrintTable(keys)
...
function killer:OnPlayerLearnedAbility( keys)
	--print ('[KILLER] OnPlayerLearnedAbility')
	--PrintTable(keys)
...
function killer:OnAbilityChannelFinished(keys)
	--print ('[KILLER] OnAbilityChannelFinished')
	--PrintTable(keys)
...
-- A player leveled up
function killer:OnPlayerLevelUp(keys)
	--print ('[KILLER] OnPlayerLevelUp')
	--PrintTable(keys)
...
function killer:OnLastHit(keys)
	--print ('[KILLER] OnLastHit')
	--PrintTable(keys)
...
function killer:OnTreeCut(keys)
	--print ('[KILLER] OnTreeCut')
	--PrintTable(keys)
...
function killer:OnRuneActivated (keys)
	--print ('[KILLER] OnRuneActivated')
	--PrintTable(keys)
...
function killer:OnPlayerTakeTowerDamage(keys)
	--print ('[KILLER] OnPlayerTakeTowerDamage')
	--PrintTable(keys)
...
function killer:OnTeamKillCredit(keys)
	--print ('[KILLER] OnTeamKillCredit')
	--PrintTable(keys)
...
function killer:Initkiller()
	killer = self
	--print('[KILLER] Starting to load killer gamemode...')
	...
	--print('[KILLER] GameRules set')

	InitLogFile( "log/killer.txt","")

	-- Event Hooks
	-- All of these events can potentially be fired by the game, though only the uncommented ones have had
	-- Functions supplied for them.  If you are interested in the other events, you can uncomment the
	-- ListenToGameEvent line and add a function to handle the event
	ListenToGameEvent('dota_player_gained_level', Dynamic_Wrap(killer, 'OnPlayerLevelUp'), self)
	ListenToGameEvent('dota_ability_channel_finished', Dynamic_Wrap(killer, 'OnAbilityChannelFinished'), self)
	ListenToGameEvent('dota_player_learned_ability', Dynamic_Wrap(killer, 'OnPlayerLearnedAbility'), self)
	...
	ListenToGameEvent('dota_item_purchased', Dynamic_Wrap(killer, 'OnItemPurchased'), self)
	ListenToGameEvent('dota_item_picked_up', Dynamic_Wrap(killer, 'OnItemPickedUp'), self)
	ListenToGameEvent('last_hit', Dynamic_Wrap(killer, 'OnLastHit'), self)
	ListenToGameEvent('dota_non_player_used_ability', Dynamic_Wrap(killer, 'OnNonPlayerUsedAbility'), self)
	ListenToGameEvent('player_changename', Dynamic_Wrap(killer, 'OnPlayerChangedName'), self)
	ListenToGameEvent('dota_rune_activated_server', Dynamic_Wrap(killer, 'OnRuneActivated'), self)
	ListenToGameEvent('dota_player_take_tower_damage', Dynamic_Wrap(killer, 'OnPlayerTakeTowerDamage'), self)
	ListenToGameEvent('tree_cut', Dynamic_Wrap(killer, 'OnTreeCut'), self)
	...
	ListenToGameEvent('dota_player_used_ability', Dynamic_Wrap(killer, 'OnAbilityUsed'), self)
	...
	--ListenToGameEvent('player_spawn', Dynamic_Wrap(killer, 'OnPlayerSpawn'), self)
	--ListenToGameEvent('dota_unit_event', Dynamic_Wrap(killer, 'OnDotaUnitEvent'), self)
	--ListenToGameEvent('nommed_tree', Dynamic_Wrap(killer, 'OnPlayerAteTree'), self)
	--ListenToGameEvent('player_completed_game', Dynamic_Wrap(killer, 'OnPlayerCompletedGame'), self)
	--ListenToGameEvent('dota_match_done', Dynamic_Wrap(killer, 'OnDotaMatchDone'), self)
	--ListenToGameEvent('dota_combatlog', Dynamic_Wrap(killer, 'OnCombatLogEvent'), self)
	--ListenToGameEvent('dota_player_killed', Dynamic_Wrap(killer, 'OnPlayerKilled'), self)
	--ListenToGameEvent('player_team', Dynamic_Wrap(killer, 'OnPlayerTeam'), self)

	-- Commands can be registered for debugging purposes or as functions that can be called by the custom Scaleform UI
	Convars:RegisterCommand( "command_example", Dynamic_Wrap(killer, 'ExampleConsoleCommand'), "A console command example", 0 )
	...
	--print('[KILLER] Done loading killer gamemode!\n\n')
end
...
function killer:PlayerConnect(keys)
	--print('[KILLER] PlayerConnect')
	--PrintTable(keys)
...
function killer:ExampleConsoleCommand()
